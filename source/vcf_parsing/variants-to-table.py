...
### DEV: Use regex instead of subsetting string when getting sample name
sample_names_list = []
...
for vcf_file in vcf_files_list:
	...
	for variant in VCF(vcf_file):
		...
		if variant.FILTER != None:
			...
			if gene_name in genes_of_interest:
				# If gene-mutation present, gene_name = 1
	...
	for gene in genes_of_interest:
		if gene in genes_sample_dict:
			...
			# If gene-mutation not present, gene_name = 0
